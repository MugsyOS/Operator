...
# New model for weight-based grinding
class WeightGrind(BaseModel):
    relay_channel: int
    target_weight: float = Field(..., gt=0)  # Target weight must be positive
...
# Updated function to grind until target weight is reached
async def grind_to_weight(gpio_pin: int, target_weight: float, timeout: int = 30):
    reader = None
    writer = None
    
    try:
        # Zero the scale before grinding
        ...
        while True:
            ...
            try:
                ...
                # If we've reached the target weight, turn off the grinder immediately
                if current_weight >= target_weight:
                    logging.info(f"Target weight reached: {current_weight} >= {target_weight}")
                    pi.write(gpio_pin, 1)  # Immediately turn off the relay
                    
                    # Attempt to send stream_stop command and close the connection gracefully
                    try:
        ...
        # Ensure the grinder is turned off (in case it hasn't been already)
